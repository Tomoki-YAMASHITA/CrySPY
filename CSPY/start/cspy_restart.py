...
def append_struc(init_struc_data):
    ...
    if rin.spgnum == 0:
        init_struc_data += gen_struc.random.rndgen.rndgen_wo_spg(nstruc, rin.natot, rin.atype, rin.nat, cID,
                                  rin.minlen, rin.maxlen, rin.dangle, rin.mindist,
                                  rin.maxcnt, rin.symtoleI, init_pos_path)
    else:
        fwpath = utility.check_fwpath()
        init_struc_data += gen_struc.random.rndgen.rndgen_spg(nstruc, rin.natot, rin.atype, rin.nat, rin.spgnum, cID,
                              rin.minlen, rin.maxlen, rin.dangle, rin.mindist,
