...
def readin():
    ...
    #---------- read input file
    if not os.path.isfile('cspy.in'):
        raise IOError('Could not find cspy.in file')
    ...
    try:
        ...
        spgnum = 'all'
    try:
        load_init_struc = config.getint('option', 'load_init_struc')
        if not (load_init_struc == 0 or load_init_struc == 1):
            raise ValueError('load_init_struc should be 0 or 1')
    except:
        load_init_struc = 0
    try:
        stop_next_struc = config.getint('option', 'stop_next_struc')
        if not (stop_next_struc == 0 or stop_next_struc == 1):
            raise ValueError('stop_next_struc should be 0 or 1')
    except:
        stop_next_struc = 0
...
def writeout():
    ...
    print('Write input data in cspy.out')
    with open('cspy.out', 'a') as fout:
        fout.write('#---------- Read cspy.in (at 1st run)\n')
        ...
        fout.write('load_init_struc = {}\n'.format(load_init_struc))
        fout.write('stop_next_struc = {}\n\n\n'.format(stop_next_struc))


def save_stat(stat):
    print('Save input data in cspy.stat')
    #---------- basic
    ...
    stat.set('input', 'load_init_struc', '{}'.format(load_init_struc))
...
def diffinstat(stat):
    ...
    old_load_init_struc = stat.getint('input', 'load_init_struc')
    old_stop_next_struc = stat.getint('input', 'stop_next_struc')
    ...
    if not old_spgnum == spgnum:
        ...
        with open('cspy.out', 'a') as fout:
            fout.write('\n#### Changed spgnum}\n')
        logic_change = True
    if not old_load_init_struc == load_init_struc:
        print('Changed load_init_struc from {0} to {1}'.format(old_load_init_struc, load_init_struc))
        print('    load_init_struc is useless when you continue simulations')
        with open('cspy.out', 'a') as fout:
            fout.write('\n#### Changed load_init_struc from {0} to {1}\n'.format(old_load_init_struc, load_init_struc))
            fout.write('####    load_init_struc is useless when you continue simulations\n')
        logic_change = True
